 ...
 * Brand: JanNigrani
 * Design: Exact match to MarketingLanding layout (White background, left-aligned content, custom landscape SVG, interactive modals)
 *
...
export default function DownloadPage() {
    ...
    // STRICT COLOR VARIABLES (JanNigrani Brand - White Background)
    const theme = {
        primary: "#111111",
        bg: "#FFFFFF",
        text: "#111111",
        ...
        <div ref={scrollRef} className="fixed inset-0 w-screen h-[100dvh] z-[9999] overflow-y-auto overflow-x-hidden font-sans flex flex-col" style={{ backgroundColor: theme.bg, color: theme.text }}>
            ...
            <main className="w-full max-w-none px-6 md:px-12 lg:px-24 pt-8 pb-24 mb-12 flex flex-col lg:flex-row items-center justify-between gap-12 relative z-10 flex-1">
                ...
                <motion.div initial="hidden" animate="visible" variants={fadeUp} className="w-full lg:w-[50%] xl:w-[45%] z-10 flex flex-col items-start justify-center text-left">
                    ...
                    <h1 className="text-[3.5rem] md:text-[4.5rem] lg:text-[5.5rem] font-black leading-[1.1] tracking-tighter mb-4 text-[#111111]">
                        {currentT.title.split(' ').map((word, i) => (
                            <React.Fragment key={i}>{word}{i % 2 !== 0 ? <br/> : ' '}</React.Fragment>
                        ))}
                    </h1>
                    
                    <p className="text-[1.1rem] md:text-[1.3rem] text-[#111111] font-medium leading-[1.6] mb-8 opacity-90">
                        {currentT.subtitle}
                    </p>

                    <div className="w-full max-w-[500px] bg-[#FFFFFF] border border-[#111111]/20 rounded-2xl p-6 mb-8 shadow-sm">
                        <div className="flex justify-between items-center mb-4 text-[0.95rem] font-bold text-[#111111]">
                            <div className="flex items-center gap-2 text-[#111111]"><HardDrive size={16} /> <span>{currentT.size}: {APP_SIZE}</span></div>
                            <div className="flex items-center gap-2 text-[#111111]"><Activity size={16} /> <span>{currentT.version}: {APP_VERSION}</span></div>
                        </div>
                        <div className="bg-[#111111]/5 p-4 rounded-xl">
                            <span className="text-[#111111] text-[0.85rem] font-bold mb-2 flex items-center gap-2"><Hash size={14} /> {currentT.checksum}</span>
                    ...
                    <a 
                        ...
                        className="w-full sm:w-auto px-12 py-5 rounded-xl font-black text-[1.2rem] transition-transform hover:scale-105 outline-none shadow-lg mb-12 flex items-center justify-center gap-3"
                    >
                        {currentT.download} <DownloadCloud size={24} />
                    </a>

                    <div className="w-full max-w-[500px]">
                        <h2 className="text-[1.4rem] font-black tracking-tight mb-6 text-[#111111]">{currentT.guideTitle}</h2>
                        
                        <div className="flex flex-col gap-5">
                            <div className="flex items-start gap-4">
                                <div className="w-8 h-8 rounded-full bg-[#111111]/5 border border-[#111111]/10 flex items-center justify-center font-bold text-[#00A9F7] flex-shrink-0 text-[0.9rem]">1</div>
                                <p className="text-[#111111] opacity-90 text-[0.95rem] leading-relaxed pt-1 font-medium">{currentT.step1}</p>
                            </div>
                            
                            <div className="flex items-start gap-4">
                                <div className="w-8 h-8 rounded-full bg-[#111111]/5 border border-[#111111]/10 flex items-center justify-center font-bold text-[#00A9F7] flex-shrink-0 text-[0.9rem]">2</div>
                                <p className="text-[#111111] opacity-90 text-[0.95rem] leading-relaxed pt-1 font-medium">{currentT.step2}</p>
                            </div>
                            ...
                            <div className="flex items-start gap-4 bg-[#00A9F7]/10 p-4 rounded-xl border-l-4 border-[#00A9F7]">
                                ...
                                <p className="text-[#111111] font-bold text-[0.95rem] leading-relaxed pt-1">{currentT.step3}</p>
                            </div>

                            <div className="flex items-start gap-4">
                                <div className="w-8 h-8 rounded-full bg-[#111111] flex items-center justify-center font-black text-[#FFFFFF] flex-shrink-0 text-[0.9rem]">4</div>
                                <p className="text-[#111111] font-bold text-[0.95rem] leading-relaxed pt-1">{currentT.step4}</p>
                            </div>
                        </div>
                    </div>
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1, delay: 0.2 }} className="w-full lg:w-[50%] h-[500px] lg:h-[750px] relative z-0 flex items-center justify-center lg:justify-end xl:pr-12">
                    <svg viewBox="0 0 600 600" className="w-full h-full max-w-[750px]" fill="none">
                        ...
                        <path d="M 0 450 L 150 220 L 300 450 Z" fill="rgba(17,17,17,0.03)" />
                        <path d="M 150 450 L 350 150 L 550 450 Z" fill="rgba(17,17,17,0.06)" />
                        <path d="M 350 450 L 480 250 L 600 450 Z" fill="rgba(17,17,17,0.04)" />

                        <path d="M -50 480 Q 150 350 350 480 T 650 480 L 650 600 L -50 600 Z" fill="rgba(17,17,17,0.08)" />
                        <path d="M -50 520 Q 200 420 400 520 T 650 520 L 650 600 L -50 600 Z" fill="rgba(17,17,17,0.12)" />
                        ...
                        >
                            <circle cx="300" cy="350" r="70" fill="rgba(0,169,247,0.05)" stroke="rgba(0,169,247,0.3)" strokeWidth="2" strokeDasharray="8 8" />
                            <circle cx="300" cy="350" r="55" fill="#FFFFFF" shadow="0 10px 30px rgba(0,0,0,0.1)" />
                            
                            <path d="M 300 320 L 300 365 M 280 345 L 300 365 L 320 345 M 280 375 L 320 375" stroke={theme.accent} strokeWidth="6" strokeLinecap="round" strokeLinejoin="round" />
                        </motion.g>

                        <path d="M 300 420 L 300 500" stroke="rgba(17,17,17,0.2)" strokeWidth="3" strokeDasharray="6 6" />
                        <circle cx="300" cy="500" r="5" fill={theme.accent} />
                    </svg>
                </motion.div>
            </main>
                ...
                {showProductsPrompt && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[9999] bg-[#111111]/60 backdrop-blur-sm flex items-center justify-center p-6">
                        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="w-full max-w-[500px] bg-[#FFFFFF] rounded-3xl p-10 flex flex-col shadow-2xl relative border border-[#111111]/10 max-h-[90vh] overflow-y-auto hide-scrollbar">
                            ...
                            <p className="text-[#111111]/70 text-[0.95rem] text-center mb-8">Part of the connected ecosystem.</p>
                ...
                <div className="flex flex-wrap justify-center items-center gap-4 text-[0.85rem] font-bold text-[#111111]">
                    ...
                    <div className="flex items-center gap-0.5 uppercase tracking-wider opacity-90">
                        ...
                        <a href="https://rebrand.ly/pbg" target="_blank" rel="noopener noreferrer" className="ml-1 hover:opacity-80 transition-opacity outline-none">
                            <img src="https://i.postimg.cc/wMgtFyQk/imageedit-8-5494402482.png" alt="PB Global Services" className="h-4 w-auto object-contain" onError={(e) => { e.target.style.display = 'none'; e.target.insertAdjacentHTML('afterend', '<span class="underline text-[#111111]">PB Global</span>'); }} />
                        </a>
                    </div>
